...
def autoencoder(x_hat, x, y, dim_img, dim_z, n_hidden, keep_prob):
    ...
    loss_sum = tf.summary.scalar('totaly_loss', loss)
    neg_marginal_likelihood_sum = tf.summary.scalar('loss_likelihood', -marginal_likelihood)
    KL_divergence_sum = tf.summary.scalar('loss_divergence', KL_divergence)

    return x_, z, loss, -marginal_likelihood, KL_divergence, loss_sum, neg_marginal_likelihood_sum, KL_divergence_sum
...
def save_sess(sess, checkpoint_dir, batch_size, step, saver=None, filename='vae', ckpt=True, frozen=False):
    filename += '.b' + str(batch_size)
    ...
    if frozen:
        tf.train.write_graph(
            tf.graph_util.convert_variables_to_constants(sess, sess.graph_def, ["generator_1/Tanh"]),
            checkpoint_dir,
            '{}-{:06d}_frz.pb'.format(filename, step),
            as_text=False)


def load(sess, checkpoint_dir, saver=None,):
    # import re
